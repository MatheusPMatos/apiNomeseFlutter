...
void setCidadeINdex(int index){
  state = state.copyWith(cidadeindex: index, status: HomePageStateStatus.initial);
}
  ...
  final result = await ref.read(ibgeServiceProvider).buscaCidades(cidade);
   ...
   switch(result){
     case Sucess(value :final cidade) :{
      if (cidade.length==1) {
         state = state.copyWith(status: HomePageStateStatus.initial, cidade: cidade.first );
      }else{
          state = state.copyWith(status: HomePageStateStatus.showmodal, listacidades: cidade, cidade: null);
      }
     }
      break;
...
Future<void> pesquisaRankingNomes () async {
  ...
  if (state.cidade == null) {
    final result = await ref.read(ibgeServiceProvider).rankingPorCidade(state.codigoCidade);
    switch(result){
     case Sucess(value :final ranking) :
      state = state.copyWith(status: HomePageStateStatus.loaded, rankingNomes: ranking);
     case Failure(exception:final e):
      state = state.copyWith(status: HomePageStateStatus.error, rankingNomes: null);
  }
  }else{
  final result = await ref.read(ibgeServiceProvider).rankingPorCidade(state.cidade!.codigo);
  switch(result){
     case Sucess(value :final ranking) :
      state = state.copyWith(status: HomePageStateStatus.loaded, rankingNomes: ranking);
     case Failure(exception:final e):
...
Future<void> pesquisaNomePorcidade(String nome) async {
  final loanderHandler = AsyncLoaderHandler.start();
  if (state.cidade != null) {
    final result = await ref.read(ibgeServiceProvider).nomesPorcidade(nome,state.cidade!.codigo);

   switch(result){
     case Sucess(value :final nomes) :
      state = state.copyWith(status: HomePageStateStatus.loaded, nomesIbge: nomes);
     case Failure(exception:final e):
      state = state.copyWith(status: HomePageStateStatus.error, nomesIbge: null);
  }
  }else{
    final result = await ref.read(ibgeServiceProvider).nomesPorcidade(nome,state.codigoCidade);
